...
REM 2. Check PostgreSQL installation
...
if "%PG_VERSION%"=="" (
    ...
    echo Please ensure PostgreSQL is installed in %PG_PATH%.
    pause
    exit /b 1
)
set "PATH=%PATH%;%PG_PATH%\%PG_VERSION%\bin"

REM 3. Create and activate Python virtual environment
if not exist ".venv" (
...
if %ERRORLEVEL% NEQ 0 (
    echo [ERROR] Failed to install Python dependencies. Please check the errors above.
    pause
    exit /b 1
)

REM 5. Setup PostgreSQL database
...
REM 6. Initialize database tables and data
